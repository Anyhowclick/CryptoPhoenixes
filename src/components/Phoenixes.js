...
import { updatePools, updateScores, updateUserFunds, fetchPhoenix } from '../actions/gameActions'
...
class Phoenixes extends Component {
  constructor(props) {
    super(props)
    this.state = {
    ...
    this.updateUserFunds = this.updateUserFunds.bind(this)
    this.updatePhoenix = this.updatePhoenix.bind(this)
    this.renderCaptains = this.renderCaptains.bind(this)
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.contract != nextProps.contract) {
      ...
      WithdrawFundsEvent = nextProps.contract.WithdrewFunds({fromBlock:'latest'}),
      ...
      NormalAbilityEvent.watch(function(err,res) {
        if (!err) {
          ...
          if (res.blockNumber > self.state.blockNumber || !self.state.trxHash.includes(res.transactionHash)) {
            ...
            self.props.dispatch(updatePools(self.props.contract))
            self.props.dispatch(updateScores(self.props.contract))
            self.updateUserFunds()
            self.updatePhoenix(phoenixID)
  ...
  updateUserFunds() {
    this.props.dispatch(updateUserFunds(this.props.contract,this.props.currAccount))
  }
  ...
  renderCaptains() {
    let phoenixes = this.props.PHOENIXES.slice(1,3)
  ...
  renderTeam(team) {
    let bg, phoenixes
    if (team == 'red') {
      phoenixes = this.props.PHOENIXES.slice(3,11)
      bg = 'redTeamBG'
    } else {
      phoenixes = this.props.PHOENIXES.slice(11,19)
      bg = 'blueTeamBG'
    }
  ...
  render() {
    return (
    <div>    
      <p id="overpayCautionText"><img src={Danger} /> Note: While overpaying is allowed, 5% of excess funds go into the Phoenix pool! <img src={Danger} /></p>
        ...
        <Row>
          <Col sm="4" />
            <Col xs="12" sm="4">
              <RainbowPhoenix
              account={this.props.currAccount}
              price={this.props.PHOENIXES[0].price}
              name={this.props.PHOENIXES[0].name}
              currentOwner={this.props.PHOENIXES[0].currentOwner}
              paused={this.state.paused}
              web3={this.props.web3}
              network={this.props.currNetwork}
              contract={this.props.contract}
...
export default connect(mapStateToProps)(Phoenixes)
